...
- [x] 1.3 实现表格对齐与删除能力及持久化联动 [#R3]
  ...
  - BUNDLE (RUN #3): Implemented R3 mutually exclusive table alignment controls with explicit marker persistence/readback, table-delete marker linkage + focus recovery, and undo restoration path; added MIXED bundle assets with CLI marker tests and GUI MCP runbook | VALIDATION_BUNDLE: auto_test_openspec/update-wysiwyg-table-editing-experience/run-0003__task-1.3__ref-R3__20260221T170724Z | HOW_TO_RUN: run.sh/run.bat
  - BUNDLE (RUN #4): Fix delete-table undo transaction to include alignment marker in the same history entry with Cmd+Z/Cmd+Shift+Z recovery path; refreshed MIXED validation assets | VALIDATION_BUNDLE: auto_test_openspec/update-wysiwyg-table-editing-experience/run-0004__task-1.3__ref-R3__20260222T015537Z | HOW_TO_RUN: run.sh/run.bat
  - EVIDENCE (RUN #4): Supervisor executed MIXED validation and confirmed alignment button mutual exclusivity, marker persistence/readback, delete confirmation + focus recovery, and undo restored table+marker even after mode switch | VALIDATED: bash auto_test_openspec/update-wysiwyg-table-editing-experience/run-0004__task-1.3__ref-R3__20260222T015537Z/tests/test_cli_table_alignment_r3.sh (exit=0; 3 tests passed); bash auto_test_openspec/update-wysiwyg-table-editing-experience/run-0004__task-1.3__ref-R3__20260222T015537Z/run.sh (server at http://127.0.0.1:33100/) + MCP assertions (alignment buttons aria-pressed mutually exclusive; markdown contained `<!-- table:align=right -->` before table; delete removed table+marker and restored focus to preceding paragraph; Cmd+Z restored table+marker in dual-pane) | RESULT: PASS | TIME: 2026-02-22T02:31:30Z
