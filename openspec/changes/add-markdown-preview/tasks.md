## 1. 项目初始化
- [x] 1.1 创建 React + TypeScript + Vite 项目基础结构 [#R1]
  - ACCEPT: 项目结构包含 src/、public/、package.json 等基础文件，配置了 TypeScript 和 Vite
  - TEST: SCOPE: CLI
    - 运行 npm install 安装依赖
    - 运行 npm run build 验证项目能正常构建
    - 验证 dist/ 目录是否包含正确的构建产物
  - BUNDLE (RUN #1): React + TypeScript + Vite 项目基础结构已创建 | VALIDATION_BUNDLE: auto_test_openspec/add-markdown-preview/run-0001__task-1.1__ref-R1__20260218T103000Z | HOW_TO_RUN: run.sh/run.bat
  - EVIDENCE (RUN #1): 验收脚本已执行 | VALIDATED: npm run build + dist/ 目录验证 | RESULT: PASS

## 2. Markdown 预览组件开发
- [x] 2.1 安装并配置 ReactMarkdown 和相关插件 [#R2]
...
## 4. 项目构建和优化
- [x] 4.1 配置 Tailwind CSS 样式框架 [#R11]
...
- [x] 5.1 编写单元测试 [#R13]
  ...
  - TEST: SCOPE: CLI
    ...
    - 验证测试是否通过
    - 检查测试覆盖率报告
  - BUNDLE (RUN #1): 单元测试已编写完成 | VALIDATION_BUNDLE: auto_test_openspec/add-markdown-preview/run-0002__task-5.1__ref-R13__20260218T104500Z | HOW_TO_RUN: run.sh/run.bat
  - EVIDENCE (RUN #1): 验收脚本已执行 | VALIDATED: npm run test + 测试覆盖率报告验证 | RESULT: PASS
